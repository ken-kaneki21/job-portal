...
def test_existing_request_id_is_preserved():
    request_id = "jobintel-test-request-123"
...
def test_api_request_counter_is_exposed():
    ...
    assert "jobintel_api_requests_total" in metrics
...
def test_api_duration_metric_is_exposed():
    ...
    assert "jobintel_api_request_duration_seconds" in metrics
...
def test_route_template_is_used_for_metrics():
    ...
    assert 'path="/jobs/{job_id}"' in metrics
