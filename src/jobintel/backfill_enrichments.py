    ...
    with SessionLocal() as session:
        ...
        if job_ids:
            existing_rows = session.scalars(
                select(JobEnrichmentRecord).where(
                    JobEnrichmentRecord.job_id.in_(job_ids)
                )
            ).all()
            ...
            try:
                ...
                if existing is None:
                    record = JobEnrichmentRecord(
                        ...
                        cloud_platforms=(enrichment.cloud_platforms),
                        data_platforms=(enrichment.data_platforms),
                        responsibilities=(enrichment.responsibilities),
                        deal_breakers=(enrichment.deal_breakers),
                    ...
                    existing.maximum_experience_years = (
                        enrichment.maximum_experience_years
                    )

                    existing.seniority = enrichment.seniority

                    existing.employment_type = enrichment.employment_type

                    existing.education = enrichment.education

                    existing.required_skills = enrichment.required_skills

                    existing.preferred_skills = enrichment.preferred_skills

                    existing.cloud_platforms = enrichment.cloud_platforms

                    existing.data_platforms = enrichment.data_platforms

                    existing.responsibilities = enrichment.responsibilities

                    existing.deal_breakers = enrichment.deal_breakers
