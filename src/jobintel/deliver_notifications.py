...
def load_pending_notifications(
    ...
    stmt = (
        ...
        .join(
            JobRecord,
            JobRecord.id == NotificationRecord.job_id,
        )
        .join(
            JobRankingRecord,
            (JobRankingRecord.job_id == NotificationRecord.job_id)
            & (JobRankingRecord.pipeline_run_id == NotificationRecord.pipeline_run_id),
        )
        .where(NotificationRecord.profile_name == PROFILE_NAME)
        .where(NotificationRecord.notification_type == NOTIFICATION_TYPE)
...
def main() -> None:
    with SessionLocal() as session:
        rows = load_pending_notifications(session)
        ...
        for notification, job, score in rows:
            ...
            idempotency_key = (
                f"jobintel/{notification.notification_type}/{notification.id}"
            )
            ...
            try:
                ...
                print()
                print(f"Failed notification {notification.id}")
