...
def main() -> None:
    with SessionLocal() as session:
        ...
        if len(run_ids) < 2:
            print()
            print(
                "Need at least two ranking runs "
                "for comparison."
            )

            if run_ids:
                print(
                    f"Current ranking run: "
                    f"{run_ids[0]}"
                )
        ...
        print(
            f"Previous high confidence: "
            f"{len(previous_ids)}"
        )

        print(
            f"Latest high confidence:   "
            f"{len(latest_ids)}"
        )

        print(
            f"New high confidence:      "
            f"{len(new_job_ids)}"
        )
